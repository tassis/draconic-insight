...
import dev.tassis.draconicinsight.integration.jade.FacilityJadeKeys;
import dev.tassis.draconicinsight.integration.jade.FacilityServerDataProvider;
...
public enum FacilityComponentProvider implements IBlockComponentProvider {
    ...
    @Override public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
        ...
        if (data.contains(FacilityJadeKeys.ENERGY)) tooltip.add(Component.translatable(
                "jade.draconic_insight.facility.energy", data.getString(FacilityJadeKeys.ENERGY),
        ...
        if (kind.equals("injector")) {
            if (!data.getBoolean(FacilityJadeKeys.LINKED)) tooltip.add(Component.translatable("jade.draconic_insight.facility.warning.unlinked"));
            if (data.contains(FacilityJadeKeys.ITEM)) tooltip.add(Component.translatable(
                    "jade.draconic_insight.facility.item", data.getString(FacilityJadeKeys.ITEM)));
            if (data.contains(FacilityJadeKeys.REQUIRED)) tooltip.add(Component.translatable(
                    "jade.draconic_insight.facility.charge", data.getLong(FacilityJadeKeys.ENERGY),
    ...
    private static void appendCoreWarnings(ITooltip tooltip, net.minecraft.nbt.CompoundTag data) {
        if (!data.getBoolean(FacilityJadeKeys.VALID)) tooltip.add(Component.translatable("jade.draconic_insight.facility.warning.core_incomplete"));
        if (!data.getBoolean(FacilityJadeKeys.STABILIZERS)) tooltip.add(Component.translatable("jade.draconic_insight.facility.warning.stabilizers_incomplete"));
    }

    private static void appendPylonState(ITooltip tooltip, net.minecraft.nbt.CompoundTag data) {
        tooltip.add(Component.translatable("jade.draconic_insight.mode." + data.getString(FacilityJadeKeys.MODE)));
        if (!data.getBoolean(FacilityJadeKeys.VALID)) tooltip.add(Component.translatable("jade.draconic_insight.facility.warning.pylon_incomplete"));
        if (!data.getBoolean(FacilityJadeKeys.LINKED)) tooltip.add(Component.translatable("jade.draconic_insight.facility.warning.unlinked"));
    }
    ...
    private static void appendFusionState(ITooltip tooltip, net.minecraft.nbt.CompoundTag data) {
        ...
        if (occupied > 0) tooltip.add(Component.translatable(
                "jade.draconic_insight.facility.injectors_occupied", injectors, occupied));
        else tooltip.add(Component.translatable("jade.draconic_insight.facility.injectors", injectors));
        if (data.getBoolean(FacilityJadeKeys.CRAFTING) && data.contains(FacilityJadeKeys.PROGRESS)) tooltip.add(Component.translatable(
                "jade.draconic_insight.facility.progress", formatPercent(data.getFloat(FacilityJadeKeys.PROGRESS))));
    }
